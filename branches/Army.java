...
public class Army extends WorldEntity {
    ...
    public String culture="roman_culture";
    public int level=0;
    public HashMap<Integer, String> armies;
 

    public Army() {

        super();
        armies = new HashMap<Integer, String>();
    ...
    public void createData(WorldMap m) {
        ...
        String file;
        if(this.canSail()){
                file = Main.DB.hashedCultures.get(culture).fleetModels.get(level);           
        }else {
                file = Main.DB.hashedCultures.get(culture).armyModels.get(level);
                 System.out.print("here:"+file);
        }
        //String file="emessairyicon.obj";
        model = map.game.getAssetManager().loadModel("models/" + file);
        Material mat1 = new Material(map.game.getAssetManager(), "Common/MatDefs/Light/Lighting.j3md");
        ...
        //can be removed, I only kept it because picking
        ...
        model.setShadowMode(ShadowMode.CastAndReceive);
        node.setLocalTranslation(map.getGLTileCenter(posX, posZ));
        model.setLocalTranslation(0f, -0.4f, 0f);
        node.attachChild(model);
