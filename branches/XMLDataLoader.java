...
public class XMLDataLoader {
    ...
    private boolean loadCulture(Element root) {
        ...
        try {
            ...
            nodes = army.getElementsByTagName("model");
            for (int i = 0; i < nodes.getLength(); i++) {
                int l = Integer.parseInt(((Element) nodes.item(i)).getAttribute("level"));
                c.armyModels.put(l, ((Element) nodes.item(i)).getAttribute("file"));
            }
            
            Element fleet = (Element) root.getElementsByTagName("fleet").item(0);
            nodes = fleet.getElementsByTagName("model");
            for (int i = 0; i < nodes.getLength(); i++) {
                int l = Integer.parseInt(((Element) nodes.item(i)).getAttribute("level"));
                c.fleetModels.put(l, ((Element) nodes.item(i)).getAttribute("file"));
            }
